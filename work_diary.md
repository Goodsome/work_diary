...
最近的工作总结：解决了两个问题。
    ...
    1. 计算压力分布的Distance有一个巨大错误，改进后发现直接计算欧式距离有一定的问题，现在还没有完全解决，可能需要借助统计学的知识。

    2. 对于MCTS节点的value评估有一定问题，之前是子节点初始化为零，选择子节点后对该节点计算value，这在之后遍历节点时对其他节点不利。改进后，像计算概率P一样，由父节点计算V，分配给每个子节点v_i。再进一步，由于P，V数据形式相同，之后可能合并为一个值，由同一个神经网络输出。
