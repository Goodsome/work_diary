...
PyOpenGL: 今天突然发现要完成作业的画图有一个很简单的函数。之前因为完全不会OpenGL，不知道如何把数据转成图像，然后各种坎坷地学习，但是都不能很好的完成作业的任务。今天又回过头看了下一个多月前看到别人的代码，就找到了那个函数glDrawPixels，然后自己写了一个简单的例子，跑成功了。接下来就是计算数据的事情了，这块内容我认为还是比较好做的，所以一直放到后面来做。感觉学习就是这样：众里寻她千百度，蓦然回首，那人却在，灯火阑珊处。
