...
GAN知识整理: 其中统计计算方面的知识不了解

## 12/17

PyOpenGL: 通过着色器实现光照漫反射

## 12/18

PyOpenGL: 今天突然发现要完成作业的画图有一个很简单的函数。之前因为完全不会OpenGL，不知道如何把数据转成图像，然后各种坎坷地学习，但是都不能很好的完成作业的任务。今天又回过头看了下一个多月前看到别人的代码，就找到了那个函数glDrawPixels，然后自己写了一个简单的例子，跑成功了。接下来就是计算数据的事情了，这块内容我认为还是比较好做的，所以一直放到后面来做。现在想想，这一个月左右学的OpenGL的时间是否值得。如果当初有人告诉我这个函数，可能作业完成的进度会快很多。不过就我个人而言，还是学到了蛮多东西。感觉学习就是这样：众里寻她千百度，蓦然回首，那人却在，灯火阑珊处。
...
用蒙特卡罗树搜索实现了一个简单的博弈问题：两人轮流取n颗石子，每次取1-m颗，取到最后一颗棋子的人获胜。
...
围棋的蒙特卡罗树搜索写完，在5x5的棋盘上也要花很多时间，迭代太少了棋力不强。

## 5/7

航空航天虚拟仿真作业：导弹弹道模拟仿真，无控运动，用4阶Ronge-Kutta法近似求解。
...
生产翼型的神经网络存在问题，回头改写围棋的神经网络。

## 6/21

蒙特卡罗树搜索+神经网络完成，下一步工作还是如何将点转为网格

## 6/22

考虑到最初随机生成的翼型几何太复杂，不好画网格，以及结果不收敛。所以先自己设定一个翼型去训练神经网络，使其输出一个较为正常的翼型。在此基础上继续训练。
...
今天写完了程序，可以跑通了，感觉是万里长征跨出了第一步。

接下来，可以开始调整神经网络，加密网格，选择cfd算法了。

## 9/10

神经网络出现问题：预训练后，输出的所有点的概率为等概率。说明神经网络还有问题。

增加神经网络层数，神经网络优化算法由momentum改为Adam，仍然没有解决问题！

## 9/11

其实上述问题属于陷入了局部最优解的问题，总结了出这个问题，接下来可以着手尝试下。

自己画的网格还是存在问题，我取出NACA例子里的坐标点，自己画了一遍网格，用的直线，6面体网格，算到10000步还是不能收敛，但是压力曲线和例子的相同。

## 9/17

openFoam的问题在于求解方程设置的问题，所以不能收敛。网格是照着例子画的，不过不太适用于自己生成的翼型。

接下来的工作是提高精度。

## 9/19

网格问题：翼型尖端，为了加密网格，采用圆弧画边长，不过结果好像差不多。

现在总结下大致有两个问题：

    1. 神经网络学习能力不强，容易陷入局部解。
    2. MCTS的搜索能力并没有体现出来。

## 10/9

这一阶段的目标：看一篇ResNet的论文，用tensorflow撸一个ResNet模型出来。

## 10/14

今天是比较关键的一天，之前一直因为用神经网络实现一个简单的映射总是容易陷入局部最小值，loss值降不下去。今天突然可以收敛了。自己反复排查了下原因，发现是神经网络编写存在一个小问题：最后的输出层加了两个激活函数，relu和softmax。就是一个relu使神经网络的梯度最后降为0。

这个问题从暑假前就一直存在，卡了我很久，我甚至打算从头再仔细学一遍神经网络和tensorflow的代码。不过也体现出了我的经验不足。今天能发现这个问题，也算是自己的幸运。

## 10/22

现在遇到两个问题，而且比较重要。

    1.翼型的mcts搜索与围棋不同，需要调整损失函数和搜索条件。
    2.训练前期产生的不规则翼型不能生产网格进行CFD计算，导致无法继续训练下去。

## 10/23

不严格的论证了画网格中采用B样条的不必要性。

找到了一种在训练过程中即使blockMesh，rhoSimpleFoam出错，也能继续训练下去的方法，而且在今后的工作中可以更进一步训练神经网络，使生成的翼型避免出错。

## 10/28

最近的工作总结：解决了两个问题。

    1. 计算压力分布的Distance有一个巨大错误，改进后发现直接计算欧式距离有一定的问题，
    现在还没有完全解决，可能需要借助统计学的知识。
    2. 对于MCTS节点的value评估有一定问题，之前是子节点初始化为零，
    选择子节点后对该节点计算value，这在之后遍历节点时对其他节点不利。
    改进后，像计算概率P一样，由父节点计算V，分配给每个子节点v_i。
    再进一步，由于P，V数据形式相同，之后可能合并为一个值，由同一个神经网络输出。

现状：程序可以比较顺利得运行，D的值会在0.6-0.8上下波动，每一轮翼型的输出分布还是少有规律，杂乱无章。不过我认为这种情况还是可以接受的，一来训练时间还太少，精度也不够，还有很多细节的地方可以调整。

今后的工作关键点在于MCTS选择节点的公式如何优化，以及神经网络的调参。后者在现阶段还不是侧重点，因为感觉比较玄，而前者可以找到一定的理论依据。
$$ \frac{P}{1+N} + Q $$

## 11/27

一个月来，进展很缓慢，现在遇到挺多问题，却不知道怎么去解决。
蒙特卡洛树搜索在非博弈问题上是如何解决的？
程序的并行问题，和tensorflow有一定的冲突。
$$ \frac{P}{1+N} + Q ​$$

# 2019/4/15

以后的日记还是应该记录起来。回顾之前的学习生活，我发现每当我能保持记录的时候，我总能够学习到一些东西，而我没有记录的那段时间，就很容易放松，去玩了。

今天开始看一本强化学习的书，400多页，坚持下去吧。现在仍没有找到一个很好的方法去看这本书，不知道怎么学习。不管怎样还是先从头看下去吧。

接下来打算把强化学习的算法都实现一边，例子采用参数化翼型。

RLAI：19-21，1.1 介绍强化学习，与监督学习和非监督学习不同；一个挑战是探索和开发，强化学习是人工智能回归简单一般原则趋势的一部分。

实现CST参数化二维翼型，sci.optimize.leastsq()最小二乘法已知翼型求参数。

## 4/16

RLAI, 22-24, 1.2 举例，介绍强化学习的例子；1.3 强化学习的元素，策略、回报、价值函数、模型；1.4 限制和范围，与进化类方法不同，各有特点和应用的地方。

整理下spining up的知识，翻译并做个ppt
